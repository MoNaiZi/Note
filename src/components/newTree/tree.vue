...
export default {
  ...
  created() {
    ...
    mittExample.on("tree-node-drag-start", (event, treeNode) => {
      console.log("接收开始拖动");
      if (
      ...
      mittExample.emit("node-drag-start", treeNode.node, event);
    });

    mittExample.on("tree-node-drag-over", (event, treeNode) => {
    ...
    mittExample.on("tree-node-drag-end", (event) => {
