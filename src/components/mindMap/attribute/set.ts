import {
    IsMdata, Mdata, SelectionCircle, SelectionG, SelectionRect,
    Transition, TspanData
} from '../interface'
...
import {
    getAddBtnClass, getAddBtnTransform, getDataId,
    getExpandBtnTransform, getGClass, getGTransform, getPath
} from './get'
...
export const attrText = (text: d3.Selection<SVGTextElement, Mdata, SVGGElement, IsMdata>, tran?: Transition): void => {
    const text2: any = text
    const t1 = tran ? text2.transition(tran) : text
    // console.log('attrText', t1)
    // debugger
    t1.attr('transform', (d: { left: any; width: number }) => {
        // console.log('d', d)
        return `translate(${d.left ? -d.width : 0},0)`
    })
}
...
export const attrPath = (
    ...
    const p1: any = p.attr('stroke', (d) => d.color).attr('stroke-width', branch)
