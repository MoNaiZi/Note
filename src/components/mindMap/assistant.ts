...
import { afterOperation, mmdata } from './data/index'
...
export const getSize = (text: string): { width: number, height: number } => {
    ...
    const { asstSvg }: { asstSvg: any } = selection
    if (!asstSvg) { throw new Error('asstSvg undefined') }
    const multiline = getMultiline(text)
    const t = asstSvg.append('text')
    t.selectAll('tspan').data(multiline).enter().append('tspan').text((d: any) => d).attr('x', 0)
...
export const moveNode = (node: SVGGElement, d: Mdata, p: TwoNumber, dura = 0): void => {
    ...
    d.px = p[0]
    d.py = p[1]
    const select: any = d3.select(node)
    select.transition(tran).attr('transform', getGTransform)
    const select2: any = d3.select(`g[data-id='${getDataId(d)}'] > path`)
    select2.transition(tran).attr('d', (d: Mdata) => getPath(d))
}

export const centerView = (): void => {
    const svg: any = selection.svg
    if (!svg) { return }
