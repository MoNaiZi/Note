...
import {
    ...
    delOne, expand, mmdata, moveChild, moveSibling, rename
} from '../data/index'
...
export const onEditBlur = (): void => {
    ...
    if (foreignEle.value && foreignDivEle.value) {
        console.log('foreignEle', foreignEle)
...
export function edit(d: Mdata, e = new MouseEvent('click')): void {
    ...
    const { g } = selection
    if (!g) { return }
    const gText = g.selectAll(`g[data-id='${getDataId(d)}'] > g.${style.content} > g.${style.text}`)
...
export function onDragMove(this: SVGGElement, e: d3.D3DragEvent<SVGGElement, Mdata, Mdata>, d: Mdata): void {
    ...
    const temp = g.selectAll('g.node').filter((other: any) => {
